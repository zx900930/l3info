	...
	while(k <= nb_arg){
		...
		if(fopen(tab[k], "r") == NULL){
			...
			if(file > -1){
				...
				commande2 = (char*) malloc((strlen("gcc -c -Wall ") + strlen(tab[k] +1))*sizeof(char));
				strcat(commande2, "gcc -c -Wall ");
				strcat(commande2, tab[k]);

				if(system(commande2) == -1){
					perror("Problème fonction systeme.\n");
					exit(-1);
				}
				free(commande2);
